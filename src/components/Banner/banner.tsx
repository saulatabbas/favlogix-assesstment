  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 flex justify-center items-center pointer-events-none">
        <div className="w-[96%] lg:w-[98%] h-[95svh]  rounded-2xl border-2 border-white/10 bg-white/4 px-6 pt-6 pb-20 backdrop-blur-[30px] sm:rounded-3xl" />
      </div>
...
<div className={`${selectedPolygon?"grid grid-cols-2  mt-[3rem]":"flex md:grid md:grid-cols-5 flex-wrap"} w-[87%]   gap-4  z-10 mx-auto   justify-between lg:hidden pointer-events-auto`}>
  ...
  .map((item, index) => (
    ...
    >
      ...
      {/* Footer / Bottom */}
      {stage !== "idle" && <BottomSection selectedPolygon={selectedPolygon} stage={stage} handlePolygonClick={handlePolygonClick} setOpenInboxPreview={setOpenInboxPreview} handlePreviewOpen={handlePreviewOpen} openInboxPreview={openInboxPreview} pageOverlay={pageOverlay} />}
...
const SelectedPolygonContent = ({openInboxPreview, selectedPolygon, stage,  }: any) => (
  <>
  <div className={`lg:relative   ${stage === "loading" ? "lg:translate-y-[10%]" : " lg:translate-y-[14%]"}`}> 
    {stage === "loading" ? (
      <motion.img src={loaderGif.src} alt={`Loading ${selectedPolygon.label} data`} className="mix-blend-screen absolute inset-0 top-[4.2%] lg:top-0 lg:relative lg:left-0 pointer-events-none object-cover size-[14rem] md:size-[18rem] mx-auto lg:size-[16vw] lg:mb-[-3%]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} />
    ) : (
 <div
  className="
    absolute lg:relative inset-0
    size-[10rem] md:size-[14rem] lg:size-[12vw]
...
<div
  className={`
    ${openInboxPreview?" z-20":" z-80"}  absolute left-[0%] ${stage === "loading"?"top-[13%] md:top-[14.5%] lg:top-[34.5%]":"top-[13%] md:top-[13%] lg:top-[25%]"}
...
> 
  ...
  <div className="block absolute left-[5%] md:top-[30%] text-center w-[90%]   lg:hidden">
    ...
    <motion.p className="roz-text   text-white font-light mx-auto text-[1rem] lg:text-[1vw] 2xl:text-[1.125rem]" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }}>
      {stage === "idle"
        ? "Please select a module to load data."
        : stage === "loading"
        ? `Retrieving and processing your ${selectedPolygon.label} data. Please wait...`
...
const BottomSection = ({setOpenInboxPreview, selectedPolygon, stage,  handlePolygonClick, handlePreviewOpen, openInboxPreview, pageOverlay }: any) => (
  <div className={`absolute w-[90%] lg:h-[40vh] ${selectedPolygon?.id === "inbox" ? "bottom-[-42%] lg:bottom-[12%]" : "bottom-[4%] lg:bottom-[10%]"}  left-0 right-0 z-20 mx-auto`}>
    {/* Header */}
    <motion.div key={stage === "loading" ? selectedPolygon?.id : "static"} variants={pageOverlay} initial={stage === "loading" ? "hidden" : false} animate={stage === "loading" ? "visible" : false}>
    ...
    {selectedPolygon?.id === "inbox" ? (
      ...
      <div className="h-[40vh] bg-white rounded-xl flex items-center flex-col justify-center border-2 border-white/10 mt-[0.5rem]  lg:mt-[1%]">
          ...
          <motion.div className="w-[95%] h-[90%] md:h-[97%] bg-white rounded-2xl overflow-hidden shadow-2xl">
            ...
            <div className="flex h-full p-[0.5%] gap-x-[0.5%]">
              ...
              <DetailsPanel open={false} />
                ...
                <BottomNav openInboxPreview={openInboxPreview} handlePreview={handlePreviewOpen} selectedPolygon={selectedPolygon?.label} handlePolygonClick={handlePolygonClick}/>
